...
class RegisterFb extends Component {
	...
	submit(values) {
		...
		this.props.dispatch(registerAsyncFb({ ...userInfo, callingCode: this.state.callingCode }));
	}
	render() {
		return (
			<View>
				<Grid>
					<Col style={{ padding: 10 }}>
						<Text style={{ color: commonColor.brandPrimary }}>{this.state.fname}</Text>
					</Col>
					<Col style={{ padding: 10 }}>
						<Text style={{ color: commonColor.brandPrimary }}>{this.state.lname}</Text>
					</Col>
				</Grid>
				<View style={{ padding: 10 }}>
					<Text style={{ color: commonColor.brandPrimary }}>{this.state.email}</Text>
				</View>
				<View
					style={{
						flexDirection: "row",
